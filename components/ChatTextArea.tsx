...
export const ChatTextArea = ({ placeholder, errors }: { placeholder: string, errors: any }) => {
    ...
    useEffect(() => {
        ...
        socketRef.current.on('response', (data: string) => {
            ...
            if (markdown.startsWith("# Convert your Markdown to PDF with PDFEquips")) {
    ...
    return (
        <div className={`chat-text-area${showTextArea ? "" : " hide"}`} onClick={(e) => {
            if (!isLoading) {
                ...
                >
                    {isLoading ? (
                        <svg className="spinner" width="16" height="16" viewBox="0 0 24 24">
                            <circle cx="12" cy="12" r="10" fill="none" strokeWidth="2" stroke="currentColor" strokeDasharray="32" strokeDashoffset="32" />
                        </svg>
                    ) : (
